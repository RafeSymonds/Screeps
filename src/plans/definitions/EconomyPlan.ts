...
const MIN_CPU_BUCKET = 2000;
const STABLE_ROOM_SKIP_CHANCE = 0.7;
...
export class EconomyPlan extends Plan {
    public override run(world: World): void {
        ...
        for (const [, worldRoom] of world.rooms) {
            ...
            const isStableRoom = pressurePenalty < 0.3 && room.storage !== undefined;

            if (cpuBucket < MIN_CPU_BUCKET && isStableRoom && Math.random() < STABLE_ROOM_SKIP_CHANCE) {
                continue;
            }
            ...
            const sourceContainers = room
                .find(FIND_STRUCTURES)
                .filter(
                    (s): s is StructureContainer => s.structureType === STRUCTURE_CONTAINER && containerIsSourceTied(s)
                );
