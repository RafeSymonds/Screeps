...
function findClosestResourceTowardsTask(
    creep: Creep,
    allResourceLocations: [string, GeneralTask.Task][],
    goalTask: GeneralTask.Task
): [number, number] {
...
export function assignCreeps(
    ...
    for (let creepIndex = 0; creepIndex < creeps.length; creepIndex++) {
        ...
        for (let taskIndex = 0; taskIndex < validTasks.length; taskIndex++) {
            let creepMatches: GeneralTask.CreepMatchesTask = validTasks[taskIndex][0].checkCreepMatches(
                creeps[creepIndex]
            );
    ...
    do {
        ...
        for (let distanceTaskIndex = 0; distanceTaskIndex < creepTaskInfos[creepIndex].length; distanceTaskIndex++) {
            ...
            if (creepMatches == GeneralTask.CreepMatchesTask.needResources) {
                let [bestTaskIndex, minDistance] = findClosestResourceTowardsTask(
                    creeps[creepIndex],
                    resourceLocations,
                    validTasks[taskIndex][0]
                );

                resourceIndex = bestTaskIndex;
                distance = minDistance;
            } else {
                distance = positionCalculations.distance(
                    creeps[creepIndex].pos,
                    validTasks[taskIndex][0].getPosition()
                );
            }
            ...
            if (validTasks[taskIndex][0].hasValueLeft()) {
                ...
                if (newElement) {
                    ...
                    if (resourceIndex != -1) {
                        ...
                        assignCreepToTask(
                            creeps[creepIndex],
                            resourceLocations[resourceIndex][1],
                            validTasks[taskIndex][0],
                            room
                        );
                    } else {
                ...
                if (distance < otherCreepDistance) {
                    ...
                    if (resourceIndex != -1) {
                        // valid resource task
                        unassignTempCreepToTask(
                            creeps[otherCreepIndex],
                            resourceLocations[resourceIndex][1],
                            validTasks[taskIndex][0],
                            room
                        );
...
export function assignCreepToTask(
    creep: Creep,
    resourceTask: GeneralTask.Task | null,
...
export function unassignTempCreepToTask(
    creep: Creep,
    resourceTask: GeneralTask.Task | null,
    task: GeneralTask.Task,
    room: Room
        ...
        if (!(structure.id in global.roomMemory[room.name].tasks)) {
            ...
            if (priority > 0) {
                ...
                if (priority === -1) {
                    ...
                    let collectEnergyTask: CollectEnergyTask = new CollectEnergyTask(
                        structure.id as unknown as Id<AnyStoreStructure>
                    );
