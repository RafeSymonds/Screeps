...
- **Node.js**: `package.json` requires Node `>=24` for the local build/test toolchain (the dev container ships Node 24). Note this is separate from the **game runtime**, which runs **Node.js 24 (V8 13.6)** as of the April 2026 server upgrade — so deployed code may use modern JavaScript natively.
